...
function chk_pay()
{
  ...
  //yjw serverMode 제거(20220225)
    pay_form.action = "/order/orderPurchaseTMS2";
  ...
  if (pay_form.enc_info.value){
	//yjw serverMode 제거(20220225)
	pay_form.action = "/order/getKcpPaymentResultTMS2";
	//pay_form.submit();
  }
}
...
<!-- 에스크로 설정 파라미터 끝-->
  ...
  <input type="hidden" name="Ret_URL"         value="/order/getKcpPaymentResultTMS2">
  <!-- 화면 크기조정 -->
  ...
  <!-- 이전페이지 파라메터 -->
	...
	<input type="hidden" class="param3" name="quantity"        	id="quantity" 		value="${order.quantity}"/>      	<!-- 수량 -->
	...
	<!-- TMS 정보 -->
	<input type="hidden" class="param3" name="tmsOrderId" id="tmsOrderId"	value='${order.tmsOrderId}'/>
	<input type="hidden" class="param3" name="orderConfigType" id="orderConfigType"	value='${order.orderConfigType}'/>
