    	...
    	<main class="mynexen">
        	<!-- header -->
          	<div class="page-header empty">
            	<h2 class="title-deco">
              		<span>무상 파손 / 조기 마모 보증 서비스 신청</span>
            	</h2>
          	...
          	<div class="order-middle-message text-center">
            	<p>무상 파손 / 조기 마모 보증 서비스 신청</p>
          	</div>


            	<div class="inset-ml">
                    <p class="h3-desc">
						보증 서비스 신청을 위해 아래와 같이 사진을 첨부해주세요.
					</p>
          	...
          	<div class="btn-wrap">
	        	<button type="button" class="btn-primary inset-m full-width" onclick="javascript:fn_insertEarlyAbrasionSrvUpload();">가입 신청</button>
	        </div>
		</main>
...
//무상 파손 보증& 조기마모 서비스 가입 신청 업로드 하기
function fn_insertEarlyAbrasionSrvUpload() {

    var formData = new FormData();
	var pUrl = "/mypage/insertEarlyAbrasionSrvUpload";

	formData.append("cMileage", $('#carMileage').val());
	formData.append("ordNo"	  , "${detailMap.ordNo}");
	formData.append("dlvrDay" , "${detailMap.dlvrDay}");
	formData.append("servCd"  , "${detailMap.servCd}");
	formData.append("dlvrSeq" , "${detailMap.dlvrSeq}");
