...
#datepicker_acdn .btnFade.btnPurple:hover {background: #6D184B;}
	            	...
	            	<article class="order-single inset-m layout-c optTb">
            			...
            			<small>
							* 얼라이먼트  정비  [50% 할인가 (국산차 /수입차 동일 가격, 4본 시 추가 할인)]
								<br>
							* <b>GO 서비스 출장비 무료 이벤트 <span style="text-decoration:line-through; text-decoration-color:red; text-decoration-thickness:2px;">50,000</span> 원</b>
						</small>
					    <div class="inset-m">
	            ...
	            <div class="accordion-info inset-ml">
            		...
            		<!-- 얼라이먼트 정비-->
	              	...
	              	<div class="submit" id="junmun" style="display: none;">
	                	...
	                	<div class="conWrap layout-r border-bottom mb-s md-trigger" name="selectAddr" val="J" onclick="callMap('1'); return false;">
		                  	...
		                  	<div class="dateTime layout-r same-size border-bottom">
		                    	<div class="inline" style="width:60%">
		                    		<img src="/img/common/ico_calendar.png" onClick="runEffect(true)" style="cursor: pointer;" />
		                      		<input type="text" name="datepicker11" class="datepicker" id="datepicker11" title="장착 희망일시 입력란" data-role="date" data-inline="true" readonly="readonly" style="width:70%;">
		                    	</div>
					                        		...
					                        		<select name="ordCoupun1_${st.count}" gbn="${st.count}" title="상품 할인쿠폰 선택">
					                          			...
					                          			<c:forEach var="icp" items="${couponItem.selectCouponItem}" varStatus="status">
						                          			<c:if test="${icp.cpnLvlCd eq '1'}">
						                          				<option value="${icp.cpnNo}" <c:if test="${icp.cpnDefYn eq 'Y' and icp.cpnGrpExpYn eq 'N'}">selected</c:if> <c:if test="${icp.cpnGrpExpYn eq 'Y'}">disabled</c:if> cpNm="${icp.cpnNm}" arrGbn="1" cpCd="${icp.cpnNo}" cpnLvlCd="${icp.cpnLvlCd}" unitCd="${icp.cpnUnitCd}" unitVal="${icp.cpnUnitVal}" dpPrNo="${icp.dpPrNo}" itemCd="${icp.itemCd}" cpnLvlCd="${icp.cpnLvlCd}" pdCpnIdx="${st.count-1}">${icp.cpnNm}<c:if test="${!empty icp.condText}">(${icp.condText})</c:if></option>
						                          			</c:if>
					                          			</c:forEach>
					                        		</select>
					                      		</dd>
			                	...
			                	<div class="border-gray-bottom">
		                 			...
		                 			<select id="selectCoupon" title="추가할인쿠폰 선택">
			                   			...
			                   			<c:forEach var="cp" items="${selectCouponPayment}" varStatus="status">
			                   				<option value="${cp.cpnNo}" <c:if test="${cp.cpnDefYn eq 'Y' and cp.cpnGrpExpYn eq 'N'}">selected</c:if> <c:if test="${cp.cpnGrpExpYn eq 'Y'}">disabled</c:if> unitCd="${cp.cpnUnitCd}" unitVal="${cp.cpnUnitVal}">${cp.cpnNm}<c:if test="${!empty cp.condText}">(${cp.condText})</c:if></option>
			                   			</c:forEach>
		                 			</select>
			                	</div>
			              	</div>
            	...
            	<article class="contents inset-ml">
              		...
              		<div class="totalPrice">
                		<div class="price mt-m">
                  			...
                  			<dl id="tireCoupon" class="layout-r sale" style="display: none">
                    			<dt>쿠폰할인(타이어)</dt>
                    			<dd><span id="tireCouponArea"></span></dd>
                  			</dl>
	...
	<div class="fullPopup md-modal-3 shop-popup md-effect-1 md-close" id="modal-Shop">
		<div class="popContents popup-dark shop-search">
			<a href="#;" class="layPopClose md-close" onclick="fn_mapClose()"></a>
		    <header>
		    	<h2>전문점 찾기</h2>
		    </header>
...
<!-- 		            		<div class="layout-c" style="display:none;"> -->
	...
	<!-- Popup Group -->
    <!-- ######################################### -->
    <div class="popupWrap md-modal md-effect-1 md-close" id="tireSizeTip" onclick="fn_Tipclose()" style="z-index: 5000; border-radius: 20px;">
	...
	<form name="jusoFrm" id="jusoFrm" method="post" >
		...
		<form id="frm" name="frm" method="post">
			...
			<input type="hidden" name="tmsOrderId" 		id="tmsOrderId" 	value=""/>								<!-- tmsOrderId-->
			<input type="hidden" name="recCustId" 		id="recCustId" 		value=""/>								<!-- 추천 회원ID -->
			<input type="hidden" name="orderConfigType" id="orderConfigType"value=""/>								<!-- orderConfigType-->
...
//전문점 찾기 팝업 > O2O 서비스 장착 전문점 한눈에 보기
        ...
        success: function (data) {
    		...
    		if(list.length > 0 ) {
	    		...
	    		//for(var i=0; i<list.length; i++) {
		    		...
		    		html += "			</div>";
		    		html += "			<div class='shop-btn-box inline text-right align-bottom layout-c'>";
		    		html += "				<button type='button' class='md-close' onClick='fn_searchAgencyInfo(\""+list[0].rentAgencyCd+"\");'><b>상세보기</b><span></span></button>";
...
// 	var drvThCd = $("input[name='drvThCd']:checked").val();			//[드라이브스루 2021-02-01] 배송상세유형 추가
	...
	$.ajax({
        ...
        success: function (data) {
    		...
    		totRowNum = (Number($("#pageNo").val())-1) * 10;

    		for(var i=0; i<list.length; i++) {
    			if(list[i].bhfCd != "O40521142" && list[i].bhfCd != "O40521145") {// O40521134 는 조회되게끔 수정
	    		html += "<li>";
	    		html += "<div class='shop-search-single'>";
	    		html += "	<div class='layout-r'>";
	    		html += "		<div class='shop-content-box layout-c'>";
	        	...
	        	if(list[i].reviewCnt > 0){
		    		html += "        <li id='nmClick' data-num='" + (i+totRowNum) + "' data-lon='" + list[i].lon + "' data-lat='" + list[i].lat +"' data-bpcd='" + list[i].bpCd + "'><b>"+list[i].bhfNm+" </b><img src='/img/product/map_star.png'/>"+list[i].reviewGrdPt+"점("+list[i].reviewCnt+")</li>";
				} else {
		    		html += "        <li id='nmClick' data-num='" + (i+totRowNum) + "' data-lon='" + list[i].lon + "' data-lat='" + list[i].lat +"' data-bpcd='" + list[i].bpCd + "'><b>"+list[i].bhfNm+"</b></li>";
				}
                ...
                html += "    	</div>";
                html += "    	<div class='shop-btn-box inline text-right align-bottom layout-c'>";
	    		html += "			<button type='button' onClick='fn_searchAgencyInfo(\""+list[i].rentAgencyCd+"\");'><b>상세보기</b><span></span></button>";
                html += "			<button type='button' onClick='moveStore(\""+list[i].lat+"\", \"" + list[i].lon + "\", \"" + list[i].bpCd + "\", \"" + (i+totRowNum) + "\");'><b>이동하기</b><span></span></button>";
...
function moveStore(agencyLat, agencyLon, bpCd, mapNum) {
...
function mapDraw(lon , lat , bhfNm , positions){
	...
	/* 마우스 휠 움직일 때 이전 맵이 잔상으로 남는 오류 때문에 맵 처음 한번만 그려야함(상단으로 올림)
	var mapContainer = document.getElementById('mapDiv'), // 지도를 표시할 div
    mapOption = {
    ...
    "onAuthorize" : function(oData) {
       	if(oData.resultCode == "Success"){
			...
			if($("#mountCd").val() == "40"){//전문점 장착
				...
				frm.action = '/order/npayComplateTMS2';
			}
			frm.submit();
       	}else{
       		ComUtil.unloading();
...
function getShopTimeList() {
	...
	param["itemCntArr"] = itemCntArr;
	param["alignYn"] = $("input[name='alignRd']:checked").val(); 
	param["amPmGbn"] = $("#amPmGbn").val(); // 2023.11.09. 정가연 추가
...
function chkVersion() {
	...
	if(mountCd != "40") {
		...
		$.ajax({
	        type: "post",
	        url: selectChkVersionTMS2, //"/order/selectChkVersion",	//
	        data: JSON.stringify(param),
	        contentType:"application/json;charset=UTF-8",  // ajax 통신으로 보내는 타입
	        async:false,
	        success: function (data) {
...
var optInfoHtml = function(optDataList){
	...
	$.each(optDataList, function(idx, optData) {
	 ...
	 if(optData.optCo != '') optHtml += "<dd>"+optData.optCo +"회</dd>";
	 else optHtml += "<dd>"+optData.cntCdNm+"회</dd>";
	 
	 optHtml += "</dl>";
	 
	 //옵션 금액 표시
	 optAmtHtml += "<dl class='layout-r' id='optionAmt'>"
	            + "<dt>"+optData.optPlcNm+"</dt>"
	            +   "<dd><span val='"+optData.optAmt+"'>"+NumUtil.setComma(optData.optAmt) +"</span></dd>"
	            +  "</dl>";
    ...
    $("#optionAmt").remove();
    $("#sidebar .totalPrice").find("dl:eq(0)").after(optAmtHtml);
}
...
var fnCalcOrderAmt = function(optTotAmt){
	...
	var tireCouponArea = $("#tireCouponArea").text() == "" ? 0 : NumUtil.rmComma($("#tireCouponArea").text());
	...
	$.each($("input[name=pdOrdAmt]") , function (idx, ordObj){
		...
		let itemCd    = $(ordObj).parents("article").find("ul").attr("itemcd");
		...
		$.each($("select[name^='ordCoupun']").find("option:selected") , function(i , obj){
			...
			let pdCpnItemCd = $($("select[name^='ordCoupun']").find("option:selected")[i]).attr("itemcd");

			// 상품에 해당 하는 쿠폰만 적용한다.
			if(itemCd == pdCpnItemCd){
				if(unitcd != "0"){
					if(unitcd == "per"){
	...
	//쿠폰할인(상품)
	$("#tireCouponArea").text(NumUtil.setComma(prdCouponAmt));
	//쿠폰할인(결제)
	$("#paymentCouponArea").text(NumUtil.setComma(couponAmt));
...
// 	}
    ...
    /* 추가할인이 없을 경우에는 보여지지 않게 하고, 쿠폰할인이 있으면 금액 보여주기*/
  	...
  	let tCpVal = $("#tireCoupon").text().replace(/[^0-9]/g,'');
...
//     			var drvThCd = $("input[name='drvThCd']:checked").val();	//[드라이브스루 2021-02-08] 배송상세유형 추가
    			...
    			*/
    			paymentCouponArea = paymentCouponArea + tireCouponArea;
	...
	$("#amArea span, #pmArea span").on("click" , function(e){
		if($("[name='mounting']:checked")[0].id=="ser3") {
            var workingTime = getWorkingTime($("#ordNoCnt0").text());

            if($("input:radio[id='radioOpta_1']").is(":checked")){
                workingTime += 30;
            }

            var time = e.target.innerText.toString().split(":");

            time[1] = Number(time[1]) + parseInt(workingTime);

            time[0] = Number(time[0]) + parseInt(time[1] / 60);
            time[1] = time[1] % 60;

            var workingTimeText = time[0] + "시 " + time[1] + "분";

            $("#jobEndTimeMsg").attr("style","display:block;");
            $("#jobEndTimeMsg span").html("<i class='ico11'></i>종료예정시간: " + workingTimeText);
        } else {
...
function btn_TireSizeTip() {
	$("#tireSizeTip").addClass("md-show");
}

function fn_Tipclose() {
	$("#tireSizeTip").removeClass("md-show");
}

function getWorkingTime(qy){
    var result = 0;

    $.ajax({
        type: "GET",
        url: "/orderOrg/getWorkingTime/" + qy,
        async: false,
        success: function (data){
            if(data == null){
