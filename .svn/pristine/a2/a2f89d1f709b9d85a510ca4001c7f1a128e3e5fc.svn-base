								    	...
								    	<div class="mynexen-order-cancel-wrap">
	                                    	...
	                                    	<c:if test="${poStat == '20'}">
	                                    		<c:if test="${poRcivMthCd == '32' || poRcivMthCd == '33' || poRcivMthCd == '34' || poRcivMthCd == '35' || poRcivMthCd == '37' || poRcivMthCd == '38' || poRcivMthCd == '39' || poRcivMthCd == '40'}">
	                                    			<button onclick="javascript:ordrO2oCancel('${data.rcivMthCd}','${data.ordNo}','${URLEncoder.encode(poPatternCd,'UTF-8')}','${data.orderFinAmt}','${carModelNm}', '${data.dlvTyDtlCd1}');" type="button" class="order-cancel">주문취소<span></span></button>
	                                    		</c:if>
	                                    	</c:if>
								    	</div>
								  	</div>
              			...
              			<div class="submit border-bottom">
                			...
                			<div class="dateTime layout-r same-size">
								...
								<c:if test="${myOrdDetailInfo.chgBtnYn == 'Y'}">
									<button onclick="javascript:fn_changeDtPop('${myOrdDetailInfo.ordNo}','${data.mountCd}');" type="button" style="background-color:#801494" class="round payment"><span>변경</span></button>
								</c:if>
                			</div>
              			</div>
...
/* 		var frm = document.orderDetailFrm;
	...
	$("#amArea span, #pmArea span").on("click" , function(e){
		...
		//if(jobEndTime=="") return false; //20220204 윤종원 변경
