...
<%
	...
	System.out.println("ordr_idxx |||||||||||||||||||| "+ordr_idxx);
...
<html xmlns="http://www.w3.org/1999/xhtml" >
    ...
    <form name="pay_info" method="post" action="/mypage/updatemyOverduePayMytd">

        <input type="hidden" name="overdueNum"       value="${payResultVO.overdueNum}">
        <input type="hidden" name="ordrId"       	 value="${payResultVO.ordrId}">
        <input type="hidden" name="recpAmt"          value="${payResultVO.recpAmt}" /> <!-- 수납 금액 -->
		<input type="hidden" name="arreAmt"          value="${payResultVO.recpAmt}" /> <!-- 연체 금액 --> <!-- 수납 & 연체 금액은 같음 -->

        <input type="hidden" name="siteCd"           value="<%=g_conf_site_cd  %>">  <!-- 사이트코드 -->
        <input type="hidden" name="reqTx"            value="<%=req_tx          %>">  <!-- 요청 구분 -->
        <input type="hidden" name="usePayMethod"     value="<%=use_pay_method  %>">  <!-- 사용한 결제 수단 -->
