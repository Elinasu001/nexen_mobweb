...
<%-- 			<input type="hidden" id="meajang" value="<%= request.getParameter("meajang") %>"> --%>
		</form>
        ...
        <div class="subContentsWrap">
          <main>
            ...
            <div class="page-header">
                <h2 class="title-deco">
                    <span>전문점을</span> <span>찾아보세요.</span>
                </h2>
                <h3 class="title-deco-sub">전문점 찾기</h3>
            </div>
            --%>
            <!-- header -->
              ...
              <div class="mapInfoBox custom-position">
                <div class="mapInfoTop">
                  ...
                  <div class="search-area-wrap-rental inset-m">
                    ...
                    <button type="button" class="go-view full-width inset-s" onclick="selectO2o('1');"><b>O2O 서비스 장착 전문점 한눈에 보기</b></button>
                  </div>
                  <!-- 찾기 검색 -->
                   <button type="button" class="search-shop" onclick="selectO2oList('1'); return false;"><span></span>넥센 직영점 찾기</button>
                  <!--total-->
                </div>
              </div>
              <div class="position">
...
/* 렌탈 전문점 찾기 조회 주석처리
	if($("#getAgencyNm").val() != null && $("#getAgencyNm").val() != "null"){
        var input = document.getElementById("meajang");
        input.setAttribute("value", $("#getAgencyNm").val());
        selectStore('1');
    }
*/
        	...
        	if(list.length > 0 ) {
	    		...
	    		for(var i=0; i<list.length; i++) {
					...
					if(list[i].reviewCnt > 0){
			    		html += "        		<li><b>"+list[i].bhfNm+"</b> <img src='/img/product/map_star.png'/>"+list[i].reviewGrdPt+"점("+list[i].reviewCnt+")</li>";
					} else {
	                ...
	                html += "    			<button type='button' class='type18' onClick='fn_searchAgencyInfo(\""+list[i].rentAgencyCd+"\");'>상세보기<i class='ico15 po_10 fr'></i></button>";
	            	html +="			 </div>";
	    			html +="		</div>";
	                html += "	</div>";
	                html += "</li>";
				...
				PageUtil.fnPageMakeMoreDisplay(pageCommon.totalCount - Number(exCnt), $("#pageNo").val() , $("#pagingMap") , "selectO2o", 5);
        	}else {
	    		html += "<li>";
	    		html += "<div class='storeList text-center inset-m'>";
	    		html += "    <ul>";
		...
		// 총 갯수 계산을 위해 필요
  		var html = "";
	    	...
	    	for(var i=0; i<list.length; i++) {
				...
				$("#pageNo").val(pageNo);
				PageUtil.fnPageMakeMoreDisplay(pageCommon.totalCount - Number(exCnt), $("#pageNo").val() , $("#pagingMap") , "selectStore", 5);
...
function selectO2oList(pageNo) {
	...
	var sigungu;
	if($("#sigungu").val() != "" && $("#sigungu").val() != null) {
		sigungu = $("#sigungu option:checked").text();
	} else {
		sigungu = $("#sigungu").val();
	}
	...
	var param = {searchClCd : sido ,  searchPtnCd : sigungu, searchKeyword : $("#meajang").val() , pageNo : pageNo , o2oYn : "Y"};
        ...
        success: function (data) {
        	...
        	if(list.length > 0 ) {
	    		for(var i=0; i<list.length; i++) {
		    		...
		    		html += "    			<ul class='mb-m'>";
		        	if(list[i].reviewCnt > 0){
			    		html += "        			<li <li id='nmClick' data-lon='" + list[i].lon + "' data-lat='" + list[i].lat +"'><b>"+list[i].bhfNm+"</b> <img src='/img/product/map_star.png'/>"+list[i].reviewGrdPt+"점("+list[i].reviewCnt+")</li>";
					} else {
			    		html += "        			<li <li id='nmClick' data-lon='" + list[i].lon + "' data-lat='" + list[i].lat +"'><b>"+list[i].bhfNm+"</b></li>";
					}		    		
		    		html += "        			<li>"+list[i].addr+"</li>";
		    		/* html += "        			<li><b>수입차 경정비 가능</b></li>"; */
		    		html += "    			</ul>";
				...
				PageUtil.fnPageMakeMoreDisplay(pageCommon.totalCount - Number(exCnt), $("#pageNo").val() , $("#pagingMap") , "selectO2oList", 5);
        	}else {
	    		html += "<li>";
	    		html += "<div class='storeList text-center inset-m'>";
	    		html += "    <ul>";
