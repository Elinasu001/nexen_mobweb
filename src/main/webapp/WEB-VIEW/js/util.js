	...
	fnPageMakeMoreDisplay : function(totalCnt, curPage, pageObj, fncName, pageSize) {
		...
		if(totalCnt < 1){
			...
			if( lastPage == curPage ){
				...
				paggingStr += "<button type='button' name='' class='product-list-more full-width inset-m' onClick='javascript:"+fncName+"("+nextPage+");'><span>더보기</span></button>";
				pageObj.html(paggingStr);
				pageObj.show();
			}
...
var PageUtilImage = {
		...
		fnPageMakeDisplay : function(totalCnt, curPage, pageObj, fncName, pageSize) {
			...
			if(total < 1){
				...
				if( firstPage > PAGEBLOCK ){
					paggingStr +=  "<span><a href='javascript:"+fncName+"(1);' class='first'>first&nbsp;</a></span>";
					paggingStr +=  "<span><a href='javascript:"+fncName+"(" + prePage + ");' class='pre'>pre&nbsp;</a></span>";
				}
				...
				if( lastPage < totalPages ){
					paggingStr += "<span><a href='javascript:"+fncName+"(" + nextPage + ");' class='next'>next&nbsp;</a></span>";
					paggingStr += "<span><a href='javascript:"+fncName+"(" + totalPages + ");' class='end'>end&nbsp;</a></span>";
				}
...
var ComUtil = {
		...
		alert:function(msg, tit, btnNm, callback){
			if( !$(".popAlert").length ){
				...
				var styleChk = "background:#fff; width:90%; height:auto; text-align:center; position:absolute; left:50%; top:"+scrollTop+"px; transform: translate(-50%, -50%); z-index:9000";
				var lyAlert =
				/*'<section class="popAlert">'+*/
				'<section class="popAlert" style="'+ styleChk + '"' + '>'+
					'<header class="text-center">'+
						/*'<div class="popHead">'+tit+'</div>'+*/
						'<div class="inset-l pt-l pb-l"><p>'+msg+'</p></div>'+
					'</header>'+
					'<footer class="layout-r same-size">'+
						'<button type="button" class="inline inset-m btn-gray-dark popClose" >'+ btnNm +'</button>'+
					'</footer>'+
				'</section>';
		...
		alertClose:function(){
			$(".popAlertBg").parent().css({'overflow' : ''});
			$(".popAlert").remove();
			$(".popAlertBg").remove();
		...
		confirm:function(msg,tit,yCb,nCb,ybtnNm,nbtnNm){
			if( !$(".popConfirm").length ){
				...
				'<section class="popAlert popConfirm" style="'+ styleChk + '"' + '>'+
					...
					'</header>'+
					'<footer class="layout-r same-size">'+
						'<button type="button" class="inline inset-m btn-gray-light btnCancel" >'+ nbtnNm +'</button>'+
						'<button type="button" class="inline inset-m btn-gray-dark btnConfirm" >'+ ybtnNm +'</button>'+
					'</footer>'+
				'</section>';
		...
		confirmClose:function(){
			$(".popAlertBg").parent().css({'overflow' : ''});
			$(".md-overlay").remove();//[20221115_1] 백그라운드 불투명 배경 제거
			$(".popConfirm").remove();
			$(".popAlertBg").remove();
			//ComUtil.scrHold.using(false);
		}
