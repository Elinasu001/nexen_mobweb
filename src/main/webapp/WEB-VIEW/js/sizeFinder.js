...
function niceGradeList(pData, niceCarGrade, pParam) {
	var carGradeNm = '';
	var carGradeHtml = '';
	$(".size-tab.type-num.step-02 .car-grade select").empty();
	$(".size-tab.type-num.step-02 .car-grade select").append('<option value="">전체</option>');
	for(var i=0; i<pData.length; i++) {
		...
		$(".size-tab.type-num.step-02 .car-grade select").append(carGradeHtml);
	}
	$(".size-tab.type-num.step-02 .car-grade select").niceSelect('update');
...
function selectChoiceSizeList() {
	...
	if(tabType == 'num') {
		...
		param["carGradeNm"] = $(".size-tab.type-num.step-02 .car-grade select").val();
	} else if(tabType == 'car') {
		param["repCarClassNbr"] = car_search.choice_items[1].searchCd;
		param["carClassNm"] = car_search.choice_items[2].searchCd;
		param["fuel"] = car_search.choice_items[3].searchCd;
...
$(function(){
	...
	$("button.size-check-tip").on('click',function(){
		$("#sizeCheckTip").fadeIn();
	});
	...
	$("#findSizeNumSubmit").on('click',function(){
		...
		frm.modelNm.value = num_search.choice_items[1].searchCdNm;	// 차종명
		// 전륜
...
var size_search = {
	...
	init:function(){
		...
		$(".size-tab.type-size .size-step-front").empty();
		$(".size-tab.type-size .size-step-front").hide();
	},
	renderStep:function( stepNum ){
		$(".size-tab.type-size .size-step").empty();
		if( this.setMode == 'R' ){
			$(".size-tab.type-size .size-step-front").show();
			$(".size-tab.type-size .size-step-front").empty();
			$(".size-tab.type-size .size-step-front").append('<div class="step-ico front">전륜</div>');
			$(".size-tab.type-size .size-step-front").append('<div class="step-front-size">'+this.getSizeStr('F')+'</div>');
